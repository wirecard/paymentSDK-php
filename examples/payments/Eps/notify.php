...
use WirecardExample\Helpers\NotificationFactory;
...
// We use Monolog as logger. Set up a logger for the notifications.
    ...
    $log->pushHandler(new StreamHandler(__DIR__ . '/../../../logs/notify_error.log', Logger::ERROR));

// ## Notification status
    $payload = strval(file_get_contents('php://input'));
    //
    $notificationHelper = (new NotificationFactory())->createNotificationHelper(
        NotificationType::TYPE_EPS,
        $payload
    );
    $payload = $notificationHelper->getPayload();


// The notification are transmitted as _POST_ request and is handled via the `handleNotification` method.
    $notification = $service->handleNotification($payload);
...
// Log the notification for a successful transaction.
    if ($notification instanceof SuccessResponse) {
        ...
        echo getTransactionLink($baseUrl, $notification);
// Log the notification for a failed transaction.
    ...
    echo sprintf('Here is occurred some issues: <br /> %s', $e->getMessage());
}
